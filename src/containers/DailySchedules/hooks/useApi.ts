...
export const useGetLectureList = (academyId?: string) => {
  ...
  return {
    lectureList: data,
    lectureItems,
    refetchLectureList: refetch,
    isLoading: status === 'pending' && fetchStatus === 'fetching',
  };
};

// 강의 하나에 대한 정보
export const useGetLectureInfo = (lectureId: string) => {
  const {data, refetch, status, fetchStatus, isError, error} = useQuery(
    ScheduleQueryOptions.getLectureInfo(lectureId),
  );

  useLoadingEffect(status, fetchStatus);
  useHandleError(isError, error);

  return {lectureInfo: data, refetchLectureInfo: refetch};
};
