...
import {useReqUpdatePersonal} from '#containers/UpdateUserInfo/hooks/useApi.ts';
import {sentryCaptureException} from '#services/sentry.ts';
import {PageData, SignUpDataProps} from '#types/signup.ts';

const UpdateUserInfo = ({navigation}: {navigation: NativeStackNavigationHelpers}) => {
  ...
  const {updatePersonal} = useReqUpdatePersonal();

  const flatListData: PageData[] = [
    {
      index: 0,
      id: 'SignUpUserData',
      props: {signUpData, setSignUpData, isChangePhone: true},
    },
    {
      index: 1,
      id: 'SignUpUserPhone',
  ...
  const handleAppStateChange = async (state: AppStateStatus) => {
    ...
    if (state === 'active') {
      ...
      try {
        await smsConfirm(payload); // SMS 발송 확인
        await updatePersonal({code}); // 개인정보 업데이트
        navigation.goBack(); // 설정 메뉴로 돌아가기
      } catch (error) {
        sentryCaptureException({error, payload, eventName: 'requestUpdatePersonal'});
      }
    }
  };
  ...
  return (
    <CSafeAreaView>
      <Header title="회원정보 재인증" navigation={navigation} isBack />
      <CView>
        <CKeyboardAvoidingView>
          <ProgressStatusNumber currentNumber={currentIndex + 1} lastNumber={2} />
          <CustomFlatList
            ref={flatListRef}
            data={flatListData}
