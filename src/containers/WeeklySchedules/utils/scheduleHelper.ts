...
export const getAdjustedTimes = (scheduleList: ScheduleDefaultProps[]) => {
  ...
  if (earliestTime && latestTime) {
    // 01:30 미만이면 무조건 00:00으로 설정
    const adjustedEarliest = moment(earliestTime, 'HH:mm').isBefore(moment('01:30', 'HH:mm'))
      ? '00:00'
      : moment(earliestTime, 'HH:mm').subtract(1, 'hours').format('HH:mm');
    // 22:30 이후이면 무조건 24:00으로 설정
    const adjustedLatest = moment(latestTime, 'HH:mm').isAfter(moment('22:30', 'HH:mm'))
      ? '24:00'
