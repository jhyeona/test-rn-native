...
const ReasonCreator = ({navigation}: {navigation: NativeStackNavigationHelpers}) => {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (isCreate) {
        // 생성
        formData.append('lectureId', isCreate ? lectureId : reasonId);
      ...
      sentryCaptureException({
        error,
        payload: {reasonId, lectureId},
        eventName: isCreate ? 'requestCreateReason' : 'requestUpdateReason',
      });
    }
  };
  ...
  useEffect(() => {
    if (reasonDetails) {
      ...
      const imgListData = reasonDetails.imagePathList.map(img => {
        ...
        const imgUrl = img.replace('{aws.s3.bucket-name}', 'stickyme-dev');
